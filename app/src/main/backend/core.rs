...
pub fn extract_thumbnails(video: &str, out_dir: &str) -> Result<(), String> {
    ...
    let codec_ctx =
        CodecContext::from_parameters(input_stream.parameters()).map_err(|e| e.to_string())?;
    let mut decoder = codec_ctx.decoder().video().unwrap();

    let tb = input_stream.time_base();

    for i in 0..count {
        let target_ts = duration_us * (i as i64 + 1) / (count as i64 + 1);

        ictx.seek(target_ts, ..).map_err(|e| e.to_string())?;
        decoder.flush();
        ...
        if !got {
            return Err(format!("Failed to decode frame {}", i));
        }
    }
